...
vec4 calcLight(BaseLight base, vec3 direction, vec3 normal){
	...
	if(diffuseFactor > 0){
		...
		vec3 reflectDirecion = normalize(reflect(direction, normal));

		float specularFactor = dot(directionToEye, reflectDirecion);
...
void main(){
	...
	vec4 color = vec4(baseColor, 1);//1.0f);

	vec4 textureColor = texture(sampler, texCoord0.xy);
	if(textureColor != vec4(0,0,0,0)){	//textureColor.xyz == vec3(0,0,0)
		color *=  textureColor;
	}
